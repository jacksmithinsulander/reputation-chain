...
app.post('/api/transaction/broadcast', (req, res) => {
  ...
  // Lägg till nya transaktionen till aktuell node...
  softCoin.addTransactionToPendingList(transaction);

  // Iterera igenom alla nätverksnoder i networkNodes och anropa respektive node
  // och skicka över den nya transaktionen...
  softCoin.networkNodes.forEach(async (url) => {
    await axios.post(`${url}/api/transaction`, transaction);
  });

  res.status(201).json({ success: true, data: 'Transation är skapad och uppdaterad' });
});

app.post('/api/transaction', (req, res) => {
  // Hämta ut transaktionsobjektet ifrån body i request objektet...
...
app.post('/api/block', (req, res) => {
  ...
  if (hashIsCorrect && hasCorrectIndex) {
    ...
    res.status(400).json({ success: false, errorMessage: 'Blocket är inte godkänt', data: block });
  }
});
